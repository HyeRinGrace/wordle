...
function appStart(){
    const displayGameover = () =>{
        ...
        div.style = 
        "display:flex; justify-content:center; align-items:center; position:absolute; top:40vh; left:38vw; background-color:yellow; width:200px; height:500px;";
        //div 스타일을 바디에 넣는 작업
    ...
    const handleEnterkey = () => {
       ...
       //for 문을 이용하여 index[0][1]부터 단어 하나하나 비교하며 정답과 일치한지를 비교
        for(let i= 0; i<5; i++){
            ...
            }else if(정답글자.includes(입력글자)) block.style.backgroudcolor ="#yellow";
    ...
    const handleBackspace = (thisBlock) =>{
        ...
        const preBlock = document.querySelector(
            `.board-column[data-index='${attempts}${index-1}']` //index-1을 해줘야 index[0]부터 시작됨
        );
    ...
    const handlekeydown = (event) => {        
        ...
        if(event.key === 'Backspace') handleBackspace(); //파라미터로 thisBlock을 전달
            ...
            if(event.key === "Enter") handleEnterkey();
             ...
             //A가 65 Z가 90 고유 키값 안에서만 입력가능하게 끔 IF 문 대입
